...
class SPyNetBasicModule(nn.Module):
    ...
    def forward(self, tensor_input):
        ...
        """
        for module in self.basic_module:
            tensor_input = module(tensor_input)
        return tensor_input


class BasicConvReluModule(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, stride, padding, act_cfg="ReLU"
    ):
        super().__init__()
        self.conv = nn.Conv2d(
        ...
        if self.with_activation and act_cfg == "ReLU":
            self.act = nn.ReLU()

    def forward(self, tensor_input):
        tensor_input = self.conv(tensor_input)
        if self.with_activation:
            tensor_input = self.act(tensor_input)
        return tensor_input
